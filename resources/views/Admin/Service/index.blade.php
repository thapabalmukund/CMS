        ...
        <div class="row">
            <div class="col-lg-12 margin-tb">
                ...
                <div class="pull-right mb-2">
                    <a class="btn btn-success" href="{{ route('services.create') }}"> Create Content</a>
                </div>
            </div>
        </div>
            ...
            <tbody>
                @foreach ($service as $data)
                    <tr>
                        ...
                        {{-- //edit --}}
                        <form method="get" action="{{ route('services.edit',$data->id) }}">
                        @csrf
                        @method('edit')
                        ...
                        {{-- <a class="btn btn-primary" href="{{route('services.edit', $content->id)}}">Edit</a> --}}
                            {{-- <a class="btn btn-primary" href="{{route('services.destroy', ['id'=>$content->id])}}">Delete</a> --}}
                        
                            {{-- <form action="{{ route('services.destroy',$content->id) }}" method="post">
                                <a class="btn btn-primary" href="{{ route('services.edit',$content->id) }}">Edit</a>
                                @csrf
                                @method('DELETE')
