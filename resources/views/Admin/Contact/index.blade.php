        ...
        <div class="row">
            <div class="col-lg-12 margin-tb">
                ...
                <div class="pull-right mb-2">
                    <a class="btn btn-success" href="{{ route('contact.create') }}"> Create Content</a>
                </div>
            </div>
        </div>
            ...
            <tbody>
                @foreach ($contact as $data)
                    <tr>
                        ...
                        {{-- //edit --}}
                        <form method="get" action="{{ route('contact.edit',$data->id) }}">
                        @csrf
                        @method('edit')
                        ...
                        {{-- <a class="btn btn-primary" href="{{route('contact.edit', $content->id)}}">Edit</a> --}}
                            {{-- <a class="btn btn-primary" href="{{route('contact.destroy', ['id'=>$content->id])}}">Delete</a> --}}
                        
                            {{-- <form action="{{ route('contact.destroy',$content->id) }}" method="post">
                                <a class="btn btn-primary" href="{{ route('contact.edit',$content->id) }}">Edit</a>
                                @csrf
                                @method('DELETE')
